...
#### 2.4.1 真实数据驱动的三条规则修正（实现后补录）

第一版规则用裸子串匹配，在真实 60 条数据上误删了正常整机。修正后每条都以真实标题原文钉成回归测试：

1. **故障词必须做否定与枚举语境判断。** 实测「**无拆修**」是卖家声明*没有*拆修、
   「避免磕碰、受潮、**进水**等」是租赁须知、「支持(人为损坏、**进水**进液…)」是质保条款。
   改为逐个出现位置检查：被否定词（无/没/未/非/免，4 字窗口）覆盖、或紧跟顿号/逗号枚举分隔符的跳过。
   效果：`repair_or_fault` 21 → 0，救回 4 台正常整机。
2. **配件词只在标题前 40 字的自述范围内匹配。** 真整机常在后段列附带清单
   （「配件:品牌电池2块 充电器」「全套包装配件都在送国产皮套」），真配件一定在开头点明。
   效果：`accessory_only` 8 → 1（只剩真电池）。
3. **区分强/弱机身信号；外观类图片措辞不单独触发 review。** 「**机身**外观轻微使用痕迹」里的
   「机身」是外观描述而非单机身信号，强信号限定为 单机身/单机/裸机/主机/本体，套机胜过弱信号；
   「成色**如图**」「看图」说的是外观，在二手挂牌里几乎无处不在，若据此一律 review 会把合格样本抽空。
   效果：同一批标题 eligible(any) 9 → 14。

**刻意保留为 review 的难例**：实测有「**单机身5499，套机6299**包邮」——标题里两个配置两个价，
规则无法确定挂牌价对应哪个配置。按 §7 诚实进 review，不硬猜。

修正后真实数据（run `b1ad3465…`）：单机身中位数 **¥5249.50**(n=12)、套机 **¥6924.50**(n=2,
正确标 `insufficient_sample`)。套机显著高于单机身，符合 X-T4 真实市场结构，是规则有效性的独立旁证。
详见 `IMPLEMENTATION_REPORT.md`「统计」一节。
